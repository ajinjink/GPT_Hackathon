...
def main(email_manager, useremail, password):
    ...
    with tab1:
        ...
        for idx, thread in enumerate(st.session_state.emails):
            ...
            if (
                f"thread_{idx}" in st.session_state.ai_responses
                or st.session_state.edit_mode.get(idx, False)
            ):
                st.session_state[thread_key] = True

            with st.expander(
                f"📨 {thread['subject']} ({len(thread['messages'])}개의 메시지)",
                expanded=st.session_state[thread_key],
            ):
                ...
                # AI 답장 생성 버튼
                if f"thread_{idx}" not in st.session_state.ai_responses and st.button(
                    "AI 답장 생성", key=f"generate_{idx}"
                ):
                ...
                if f"thread_{idx}" in st.session_state.ai_responses:
                    if not st.session_state.edit_mode.get(idx, False):
                        ...
                        if f"textarea_{idx}" not in st.session_state:
                            st.session_state[f"textarea_{idx}"] = (
                                st.session_state.edited_texts.get(
                                    f"thread_{idx}",
                    ...
                    with col1:
                        if st.button("답장 전송", key=f"send_{idx}"):
                            with st.spinner("답장을 전송중입니다..."):
                                ...
                                success = handle_email_send(
                                    idx, thread, email_manager, useremail, password
                                )
                                if success:
                                    # 메시지를 스레드에 추가
                                    new_message = {
                                    ...
                                    st.session_state.emails[idx]["messages"].append(
                                        new_message
                                    )
                                    ...
                                    if f"thread_{idx}" in st.session_state.edited_texts:
                                        del st.session_state.edited_texts[
                                            f"thread_{idx}"
                                        ]
                    ...
                    with col2:
                        if st.button("다시 생성", key=f"regenerate_{idx}"):
                            with st.spinner("AI가 답장을 다시 생성중입니다..."):
                                ...
                                st.session_state.edited_texts[f"thread_{idx}"] = (
                                    new_response
                                )
                                # 수정 모드였다면 초기화
                                st.session_state.edit_mode[idx] = False
                    ...
                    with col3:
                        ...
                        if st.button(edit_button_text, key=f"edit_{idx}"):
                            current_edit_mode = st.session_state.edit_mode.get(
                                idx, False
                            )
